...
public static class Program
{
  ...
  {
    Data.TruckDealers.Clear();

    HashSet<(string Make, Region Region)> truckDealersWithoutFerries = [];
    HashSet<(string Make, Region Region)> truckDealersWithFerries = [];

    foreach ((string city, string make) in Data.TruckDealersByCity)
    {
      string cityName = city;
      if (!Data.Cities.ContainsKey(city))
      {
        if (Data.CityAliases.ContainsKey(city)) cityName = Data.CityAliases[city];
        else
        {
          Console.Error.WriteLine($"Unrecognized city: {city}");
          continue;
        }
      }

      Check cityData = Data.Cities[cityName];

      if (cityData.FerryRequired)
        truckDealersWithFerries.Add((make, cityData.Region));
      else
        truckDealersWithoutFerries.Add((make, cityData.Region));
    }

    foreach ((string make, Region region) in truckDealersWithoutFerries)
    {
      Data.TruckDealers.Add(new Check(
        Name: make,
        Region: region,
        FerryRequired: false
      ));
    ...
    foreach ((string make, Region region) in truckDealersWithFerries.Except(truckDealersWithoutFerries))
    {
      Data.TruckDealers.Add(new Check(
        Name: make,
        Region: region,
    ...
    Data.TruckDealers.FormatCSVToFile(string.Format(Data.CsvPath, Data.TruckDealersFN),
      check => new Dictionary<string, string?>
      {
        ["Make"] = check.Name,
        ["State"] = check.Region.StateName,
        ["DLC"] = check.Region.DLCName,
        ["FerryRequired"] = check.FerryRequired ? "true" : ""
      }
    );
  }
}
